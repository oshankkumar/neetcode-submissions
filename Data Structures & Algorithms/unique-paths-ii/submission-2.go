...
func findUniquePathsWithObstacles(i, j int, obstacleGrid [][]int, memo [][]int) int {
	...
	var count int
	if i+1 < len(obstacleGrid) && obstacleGrid[i+1][j] == 0 {
		count += findUniquePathsWithObstacles(i+1, j, obstacleGrid, memo)
	}

	if j+1 < len(obstacleGrid[0]) && obstacleGrid[i][j+1] == 0 {
		count += findUniquePathsWithObstacles(i, j+1, obstacleGrid, memo)
	}
